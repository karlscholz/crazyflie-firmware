...
LOG_GROUP_START(UWB_A)
LOG_ADD(LOG_UINT16, update, &state.update[0])
LOG_ADD(LOG_UINT16, distance, &state.distance[0])
LOG_ADD(LOG_FLOAT, velX, &state.velX[0])
LOG_ADD(LOG_FLOAT, velY, &state.velY[0])
LOG_ADD(LOG_FLOAT, gyroZ, &state.gyroZ[0])
LOG_ADD(LOG_FLOAT, height, &state.height[0])
LOG_GROUP_STOP(UWB_A)
LOG_GROUP_START(UWB_B)
LOG_ADD(LOG_FLOAT, myVelX, &state.myVelX)
LOG_ADD(LOG_FLOAT, myVelY, &state.myVelY)
LOG_ADD(LOG_FLOAT, myGyroZ, &state.myGyroZ)
LOG_ADD(LOG_FLOAT, myHeight, &state.myHeight)
LOG_GROUP_STOP(UWB_B)