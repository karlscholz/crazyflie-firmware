  ...
  while(1) {
    ...
    if(relative_localization((float *)rlVarForCtrl) && keepFlying) {
      ...
      if (timeInAir >= 30000) {
        ...
        // coordinate transformation from mine (rlVarForCtrl) to leaders coord system
...
LOG_GROUP_START(Lead_POS)
LOG_ADD(LOG_FLOAT, LeadX, &rlVarForCtrl[0][STATE_rlX])
LOG_ADD(LOG_FLOAT, LeadY, &rlVarForCtrl[0][STATE_rlY])
LOG_ADD(LOG_FLOAT, LeadYaw, &rlVarForCtrl[0][STATE_rlYaw]) 
LOG_GROUP_STOP(Lead_POS)